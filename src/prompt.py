...
def _build_tool_narration_section() -> str:
    return (
        ...
        "Do NOT speak your own filler phrase before those four "
        "tools — just invoke the tool. The runtime filler covers the execution "
        "latency for you, and if you also speak your own filler the two will "
        "overlap and cut each other off mid-sentence on the caller's line.\n"
        "\n"
        "For any OTHER tool, or when no runtime filler applies, you MUST still "
        "speak a short, natural acknowledgement before invoking it. Tools take "
        "one to three seconds to run, and silence on a live phone call feels "
        ...
        "4. Speaking acknowledgement is a contract. If you speak it but do not "
        "actually invoke the tool in the same turn, you have lied to the caller — "
        "see OUTCOME WORDS. Filler without a real tool call is worse than silence.\n"
        "\n"
        ...
        "phone call. Filler-without-tool-call is the worst. Double-filler — your "
        "own phrase overlapping the runtime's — is the third-worst."
    )
