...
def _build_tool_narration_section() -> str:
    return (
        ...
        "The tool runtime automatically plays a short acknowledgement phrase "
        "when you invoke one of the four scoped tools "
        "(check_availability, book_appointment, capture_lead, or transfer_call). "
        "Do NOT speak your own filler phrase before those four tools — just "
        "invoke the tool. The runtime filler covers the execution latency for "
        "you, and if you also speak your own filler the two will overlap and "
        "cut each other off mid-sentence on the caller's line.\n"
        "\n"
        "For any OTHER tool, or when no runtime filler applies, you MUST still "
        "speak a natural acknowledgement before invoking it. Tools take one to "
        "three seconds to run, and silence on a live phone call feels broken — "
        "if you go silent, the caller often says 'Hello?' which cancels the "
        "in-flight tool and restarts the whole turn.\n"
        "\n"
        "Rules:\n"
        "1. For check_availability, book_appointment, capture_lead, and "
        "transfer_call: invoke the tool directly — the runtime speaks for you. "
        "Do not generate your own filler text for these four.\n"
        "2. For any other tool call, never emit the call without speaking first.\n"
        "3. Any acknowledgement you do speak must be natural and conversational — "
        "not 'please hold' (too cold) or 'one moment please' (too formal).\n"
        "4. AIM FOR ~3 SECONDS of speech (not 1 second). A two-word filler like "
        ...
        "5. Speak the acknowledgement, then immediately invoke the tool. Do not "
        "wait for the caller to reply.\n"
        "6. Speaking acknowledgement is a contract. If you speak it but do not "
        "actually invoke the tool in the same turn, you have lied to the caller "
        "— see OUTCOME WORDS. Filler without a real tool call is worse than "
        "silence.\n"
        "\n"
        "Fallback tone guidance (only if no runtime filler is playing for you):\n"
        ...
        "live phone call. Filler-without-tool-call is the worst. Double-filler "
        "— your own phrase overlapping the runtime's — is the third-worst."
    )
