...
def _build_booking_section(business_name: str, onboarding_complete: bool, postal_label: str) -> str:
    ...
    return (
        ...
        "date that has passed or a time too soon, let them know and guide them forward.\n"
        "\n"
        "The goal is to learn the caller's preferred day and time, then verify that specific slot. "
        "Ask the caller when works for them. If they give a day without a time, ask what time "
        "works best. If they give a time without a day, ask which day. Once you have both, "
        "check that exact slot with check_availability.\n"
        "\n"
        "Never read out or list available time slots — even if the caller asks 'what's available?' "
        "or 'do you have any slots?'. Instead, confirm that the day has availability and ask what "
        "time of day they prefer. The caller chooses the time; you verify it.\n"
        "\n"
        "If their preferred time is available, proceed to book. If not, offer only the 2-3 closest "
        "alternatives from the results. If no slots remain on their preferred day, ask if another "
        "day works. Every new date or time the caller mentions requires a fresh check_availability "
        "call — never rely on earlier results.\n"
        "\n"
