...
def create_capture_lead_tool(deps: dict):
    ...
    async def capture_lead(
        ...
        # Runtime-owned latency cover (lib/tool_filler): the record_call_outcome
        # RPC plus an optional validation fallback are Tokyo round-trips.
        async with tool_filler(context, deps, "lead"):
            return await _capture_lead_impl(
                deps, context, caller_name, phone, street_name, unit_number,
